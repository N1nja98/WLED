...
action="/settings/time"><button type="submit">Time & Macros</button></form><form
 action="/settings/clock"><button type="submit">Clock</button></form><form 
...
// Autogenerated from wled00/data/settings_clock.htm, do not edit!!
const char PAGE_settings_clock[] PROGMEM = R"=====(<!DOCTYPE html><html><head><meta name="viewport" content="width=500"><meta 
charset="utf-8"><title>Clock</title><script>
function H(){window.open("https://github.com/Aircoookie/WLED/wiki/Settings#security-settings")}function B(){window.open("/settings","_self")}function GetV() {var d=document;
%CSS%%SCSS%</head><body onload="GetV()">
<form id="form_s" name="Sf" method="post"><div class="helpB"><button 
type="button" onclick="H()">?</button></div><button type="button" onclick="B()">
Back</button><button type="submit">Save</button><hr><h2>Clock Settings</h2>
Turn on Clock: <input type="checkbox" name="TOC"><br>Clock brightness: <input 
id="clock_brightness" name="CB" type="number" min="0" max="255" required>
 (0-255)<br><br>Clock Hex Color: <input id="clock_color" maxlength="9" 
...
name="AD"><br>-or-<br>Dim clock from: <input type="time" name="DCF"> to: <input 
type="time" name="DCT"><br><br><h3>Formatting</h3>Time Format: <select 
name="TF"><option value="0" selected="selected">HH:MM</option><option value="1">
HH:MM:SS</option><option value="2">HH:MM AM/PM</option><option value="3">
The time is HH:MM AM/PM</option></select><br>Date Format: <select name="DTF">
<option value="0">MM/DD</option><option value="1">MM/DD/YY</option><option 
value="2">MM/DD/YYYY</option><option value="3">Day, MM/DD/YY</option><option 
value="4">Day, MM/DD/YYYY</option><option value="5">Day, Month Date, Year
</option><option value="6">Today is Day, Month Date, Year</option></select><br>
